...
describe('petstore', () => {
  ...
  it('uses path segments as the query key', async () => {
    ...
    expect(
      /queryKey: \['pets', '\{petId\}'\]|queryKey: \["pets", "\{petId\}"\]/.test(queries),
    ).toBe(true);
  });
  ...
  it('passes bare client specifiers through untouched', async () => {
    const requests = fileNamed(
      await run('petstore.yaml', { client: '@/lib/api-client' }),
      'requests.ts',
    )!;
...
describe('edge cases', () => {
  ...
  it('suffixes duplicate operationIds and warns', async () => {
    ...
    expect(result.warnings.some((w) => w.includes('Duplicate operation name'))).toBe(
      true,
    );
  });
