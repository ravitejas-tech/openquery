...
How OpenQuery turns an OpenAPI document into typed React Query hooks, and why it is
...
OpenQuery is a four-stage pipeline. Each stage is pure — it takes data in and returns
...
All output carries a `// Generated by OpenQuery — do not edit.` header and is
...
OpenQuery ships **no runtime**. Generated code imports only from `react-query-kit`,
...
So OpenQuery does not guess. With no configuration, **no infinite queries are
