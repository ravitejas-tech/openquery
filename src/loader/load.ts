...
export async function loadSpec(input: string, root: string): Promise<LoadedSpec> {
    ...
    try {
        ...
        throw new SpecError(error instanceof Error ? error.message : 'Failed to load specification.', {
            file: source,
            cause: error,
    ...
    if (first) {
        throw new SpecError(String(first.message), {
            ...
            hint: errors.length > 1 ? `${errors.length - 1} more error(s) in this document.` : undefined,
        })
    }
