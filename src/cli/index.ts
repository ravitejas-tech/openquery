...
const ESC = '\x1b'
const wrap = (open: string, close: string) => (s: string) => (useColor ? `${ESC}[${open}m${s}${ESC}[${close}m` : s)
const color = {
    dim: wrap('2', '22'),
    red: wrap('31', '39'),
    green: wrap('32', '39'),
...
function reportResult(result: GenerateResult, options: { dryRun: boolean; silent: boolean }): void {
    ...
    if (changed.length === 0) {
        console.log(`${color.green('✓')} Up to date ${color.dim(`(${result.operationCount} operations)`)}`)
        return
    }
...
cli.command('generate', 'Generate typed hooks from an OpenAPI document')
