...
import type { HttpMethod, IR, IRSchema, Operation, Param, ParamLocation, PaginationSpec } from './types.js'
import { SchemaConverter } from './schema.js'
import { NameRegistry, deriveOperationId, safeIdentifier, toPascalCase } from './naming.js'
...
function pickJsonContent(content: Record<string, any> | undefined): { schema: any; contentType: string } | undefined {
    if (!content) return undefined
    const types = Object.keys(content)
    // Prefer JSON; fall back to whatever single type is offered.
    const jsonType = types.find((t) => t === 'application/json') ?? types.find((t) => /\+json$/.test(t)) ?? types[0]
...
function successResponse(responses: Record<string, any> | undefined): any | undefined {
    ...
    const okCode =
        codes.find((c) => c === '200') ?? codes.find((c) => /^2\d\d$/.test(c)) ?? codes.find((c) => c === 'default')
...
function resolvePagination(
    ...
    const extension = rawOperation['x-queryfish-pagination'] as PaginationConfig | undefined
...
export function buildIR(document: Record<string, any>, config: QueryFishConfig): IR {
    ...
    for (const [routePath, pathItem] of Object.entries(paths as Record<string, any>)) {
        ...
        for (const method of METHODS) {
            ...
            // Parameters -------------------------------------------------------
            const allParams: any[] = [...sharedParams, ...(Array.isArray(raw['parameters']) ? raw['parameters'] : [])]
            ...
            const swaggerBody = allParams.find((p) => p && typeof p === 'object' && p['in'] === 'body')
            ...
            const cursorParam = pagination ? queryParams.find((p) => p.name === pagination.param) : undefined
