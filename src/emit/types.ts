    ...
    switch (schema.kind) {
        ...
        case 'intersection':
            return schema.members
                .map((m) => (needsParens(m) ? `(${renderType(m, indent)})` : renderType(m, indent)))
                .join(' & ')
...
function renderObject(schema: Extract<IRSchema, { kind: 'object' }>, indent: string): string {
    ...
    for (const property of schema.properties) {
        const doc = jsdoc({ description: property.description, deprecated: property.deprecated }, inner)
        if (doc) lines.push(doc.trimEnd())
        const optional = property.required ? '' : '?'
        lines.push(`${inner}${quotePropertyIfNeeded(property.name)}${optional}: ${renderType(property.schema, inner)};`)
    }
...
export function emitTypes(ir: IR): string {
    ...
    for (const operation of ir.operations) {
        ...
        blocks.push(`export type ${responseTypeName(operation)} = ${renderType(operation.response)};`)
    }
